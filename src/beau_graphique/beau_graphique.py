...
def slide(elements: list, nrows=3, ncols=4,
    ...
    for elem in elements:
        ...
        couleur = elem.get("couleur", PALETTE[0])
        ...
        if typ == "vide":
            ...
            ax.text(ha_txt, 0.5, texte, transform=ax.transAxes,
                    ha=align, va="center",
                    fontsize=taille, fontweight="bold", color=_T["texte"],
                    wrap=True)
            ...
            ax.text(ha_txt, 0.5, texte, transform=ax.transAxes,
                    ha=align, va="center",
                    fontsize=taille, color=_T["texte_dim"],
                    wrap=True)
            ...
            ax.text(0.10, 0.5, texte, transform=ax.transAxes,
                    ha="left", va="center",
                    fontsize=taille, color=_T["texte"],
            ...
            col_delta = _COULEUR_HAUT if positif else _COULEUR_BAS
...
def _dessiner_barre_tendance(ax, valeurs, deltas, cumul, couleur, nom=""):
    ...
    ax.tick_params(axis="y", labelsize=6.2, colors=_T["texte_dim"], length=2, pad=2)
    ...
    ax.axhline(0, color=_T["grille"], linewidth=0.6, zorder=1)

    if nom:
        wrapped = "\n".join(textwrap.wrap(nom, width=18))
        ax.set_xlabel(wrapped, fontsize=7.5, color=_T["texte"], labelpad=5)
...
def tendances_grille(
    ...
    **_extra,
    ...
    nrows    = (n + ncols - 1) // ncols
    fig_size = _resoudre_figsize(figsize, format) or (ncols * 3.4, nrows * 3.6)
    ...
    for idx, item in enumerate(items):
        ...
        _dessiner_barre_tendance(
            ...
            couleur=item.get("couleur", PALETTE[idx % len(PALETTE)]),
            nom=item.get("nom", ""),
        )
...
def _dessiner_tendance_cat(ax, x_base, valeurs, deltas, cumul, couleur,
    ...
    for xi, hi in [(xl, h0), (xm, h1), (xr, h2)]:
        ax.plot(xi, hi, "o",
                markersize=7, markerfacecolor=_T["bg"],
...
def tendances_comparatives(
    ...
    **_extra,
    ...
    fig_h = nrows * 3.5
    fig_size = _resoudre_figsize(figsize, format) or (fig_w, fig_h)
    ...
    for row_idx in range(nrows):
        ...
        for col_idx, item in enumerate(row_items):
            _dessiner_tendance_cat(
                ...
                deltas=item.get("deltas", []),
                cumul=item.get("cumul", 0),
                couleur=item.get("couleur", PALETTE[col_idx % len(PALETTE)]),
                cat_w=cat_w, ymax_ax=ymax,
                nom=item.get("nom", ""),
            )

        ax.grid(False)
        ax.axhline(0, color=_T["grille"], linewidth=0.8, zorder=1)
        ax.spines["top"].set_visible(False)
        ax.spines["right"].set_visible(False)
        ax.spines["bottom"].set_visible(False)
        ax.spines["left"].set_color(_T["grille"])
        ax.spines["left"].set_linewidth(0.6)
        ax.tick_params(axis="y", labelsize=8, colors=_T["texte_dim"], length=2, pad=2)
        ax.tick_params(axis="x", bottom=False, labelbottom=False)
    ...
    if sous_titre:
        fig.text(0.03, y_cur, sous_titre,
                 fontsize=8.5, color=_T["texte_dim"], ha="left", va="top")
